...
bool MenuEntry::handle_event(SDL_Event& event)
{
	...
	return((event.type == SDL_KEYDOWN &&
				(key.sym == SDLK_RETURN || key.sym == SDLK_KP_ENTER)) || (event.type == SDL_JOYBUTTONDOWN &&
				(event.jbutton.button == XBOX_BUTTON_B || event.jbutton.button == XBOX_BUTTON_CLICK1)) ||
			event.type == SDL_MOUSEBUTTONUP);
#else
	return((event.type == SDL_KEYDOWN &&
				(key.sym == SDLK_RETURN || key.sym == SDLK_KP_ENTER)) || (event.type == SDL_MOUSEBUTTONUP));
#endif
}
		...
		while (SDL_PollEvent(&event))
		{
			...
			case XBOX_BUTTON_B:
			  ...
			  case XBOX_BUTTON_Y:
				...
				fakeevent.key.keysym.sym = SDLK_y;
				fakeevent.key.keysym.unicode = 0x59;
			  ...
			  case XBOX_BUTTON_X:
				...
				fakeevent.key.keysym.unicode = 0x58;
				fakeevent.key.keysym.mod = KMOD_NONE;
				SDL_PushEvent (&fakeevent);
				break;
			case XBOX_BUTTON_START:
				fakeevent.type = SDL_KEYDOWN;
				fakeevent.key.type = SDL_KEYDOWN;
				fakeevent.key.keysym.sym = SDLK_KP_ENTER;
				SDL_PushEvent (&fakeevent);
				break;
			...
			else if((SDL_JOYHATMOTION) && (event.jhat.value != SDL_HAT_CENTERED))
			{
			//mouse->hide();
			//mouse->blit_dirty();
			switch(event.jhat.value) {
			case SDL_HAT_UP:
			  //printf ("Evento arriba\n");
				if (!selected)
				{
					// if unselected (by 'MouseOut' event), just re-select
					set_selection(selection);
...
#ifdef XENON
		if ((event.type == SDL_JOYAXISMOTION) && (( event.jaxis.value > -7000 ) || ( event.jaxis.value < 7000 )))
		{
			...
			if( event.jaxis.which == 0 ) 
			{
				...
				if( event.jaxis.axis == 0 )
				{
					...
					if( ( event.jaxis.value > -7000 ) && ( event.jaxis.value < 7000 ) ) 
					{
						q = 0;
					}
					//If not 
					...
					{
						q = (event.jaxis.value / 2000);
					} 
				} 
				//If the Y axis changed else 
			...
			y = y + r;
			SDL_WarpMouse(x, y);
			//if ((rand() %3) == (1)){std::cerr << "increment x = " << q << " increment y = " << r  << " mouse moved to x:" << x << " y:" << y << std::endl;}
